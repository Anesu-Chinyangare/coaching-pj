// ─────────────────────────────────────────
//  src/server.js — Nexus CRM Entry Point
// ─────────────────────────────────────────
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://www.google-analytics.com", process.env.SUPABASE_URL],
    }
  }
}));
...
const server = app.listen(PORT, () => {
  logger.info(`Nexus CRM running on port ${PORT} [${process.env.NODE_ENV}]`);
});
