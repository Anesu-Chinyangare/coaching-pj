...
const getLeads = async (req, res, next) => {
  try {
    ...
    let query = supabase.from('leads').select('*', { count: 'exact' }).order('score', { ascending: false }).order('created_at', { ascending: false }).range(offset, offset + parseInt(limit) - 1);
...
const createLead = async (req, res, next) => {
  try {
    const { data, error } = await supabase.from('leads').insert({ ...req.body, owner_id: req.user?.id }).select().single();
    if (error) throw error;
    await supabase.from('activities').insert({ entity_type: 'lead', entity_id: data.id, user_id: req.user?.id, action: 'created', metadata: { source: data.source, stage: data.stage } });
...
const convertLead = async (req, res, next) => {
  try {
    ...
    const { data: customer, error: custErr } = await supabase.from('customers').insert({ first_name: lead.first_name, last_name: lead.last_name, email: lead.email, phone: lead.phone, company: lead.company, owner_id: lead.owner_id, status: 'active' }).select().single();
