...
const getCustomer = async (req, res, next) => {
  try {
    ...
    const [customerRes, appointmentsRes, leadsRes, activitiesRes] = await Promise.all([
      supabase.from('customers').select('*, owner:users(id, name)').eq('id', id).single(),
      ...
      supabase.from('activities').select('*, user:users(id, name)').eq('entity_type', 'customer').eq('entity_id', id).order('created_at', { ascending: false }).limit(20),
    ]);
...
const createCustomer = async (req, res, next) => {
  try {
    ...
    await supabase.from('activities').insert({
      entity_type: 'customer', entity_id: data.id,
...
const updateCustomer = async (req, res, next) => {
  try {
    ...
    await supabase.from('activities').insert({
      ...
      user_id: req.user?.id, action: 'updated', metadata: req.body,
    });
